...
import { Package } from "../types/storage";
...
export function getUpdatePackageInfo(
  ...
  if (updatePackage.rollout && isUnfinishedRollout(updatePackage.rollout)) {
...
function getUpdatePackage(
  ...
  for (let i = packageHistory.length - 1; i >= 0; i--) {
    ...
    if (
      ...
      (ignoreRolloutPackages &&
        (packageEntry.rollout ?? isUnfinishedRollout(packageEntry.rollout!)))
    ) {
      continue;
    }
    ...
    if (
      !request.isCompanion &&
      !semver.satisfies(request.appVersion, packageEntry.appVersion)
    ) {
      continue;
    }
  ...
  if (!latestEnabledPackage) {
    ...
    (latestSatisfyingEnabledPackage &&
    ...
    if (semver.gtr(request.appVersion, latestEnabledPackage.appVersion)) {
      ...
      !semver.satisfies(request.appVersion, latestEnabledPackage.appVersion)
    ) {
  ...
  } else if(latestSatisfyingEnabledPackage) {
